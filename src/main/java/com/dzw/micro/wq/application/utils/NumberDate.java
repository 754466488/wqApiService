...
public class NumberDate {
	...
	public int getMonth() {
		return Integer.parseInt(this.toString().substring(4, 6));
	}

	public int getDay() {
		return Integer.parseInt(this.toString().substring(6, 8));
	}

	public int getHour() {
		return Integer.parseInt(this.toString().substring(8, 10));
	}

	public int getMinute() {
		return Integer.parseInt(this.toString().substring(10, 12));
	}

	public int getSecond() {
		return Integer.parseInt(this.toString().substring(12, 14));
	}
	 ...
	 * 是否相等
	 *
	 * @param another   - 要减的时间数
	 * @param transType - 按照指定类型
	 * @return
	 */
	public boolean equals(NumberDate another, TransType transType) {
		Preconditions.checkNotNull(another);
		return this.diff(another, transType) == 0L;
	}
