...
/**
 * 接口接收请求日志打印
 * 日志格式：应用名称|本机IP|httpReceive|requestURI|入参|结果|耗时毫秒
 *
 * @author lyb
 * @date 2020/11/26
 */
@Component
@Order(99)
public class ApiLogger implements AspectHandler {
	@Autowired
	...
	public Resp preHandle(AspectInfo aspectInfo) throws Throwable {
		...
		sb.append(AspectTypeEnum.HTTP_RECEIVE.getName()).append(SymbolConstant.BAR);
		sb.append(WebUtils.getRequestURI()).append(SymbolConstant.BAR);
	...
	public void errorHandle(AspectInfo aspectInfo, Throwable throwable) {
		...
		errorSB.append(WebUtils.getRequestURI()).append(SymbolConstant.BAR);
